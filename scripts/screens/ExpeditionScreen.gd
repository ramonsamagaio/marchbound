...
func _build() -> void:
	...
	var side=PanelContainer.new(); side.custom_minimum_size.x=300; side.add_theme_stylebox_override("panel",UIFactory.panel(Color("#151b2a"),12,Color("#35415d"))); layout.add_child(side); var hud=VBoxContainer.new(); side.add_child(hud); hud.add_child(UIFactory.title(tile.biome,22)); hud.add_child(UIFactory.label("Threat %d%s"%[tile.threat," · BOSS TERRITORY" if tile.boss else ""],13,Color("#e0b58d"))); hud.add_child(UIFactory.hsep()); hp_bar=ProgressBar.new(); hp_bar.custom_minimum_size.y=26; hp_bar.show_percentage=false; hud.add_child(hp_bar); status=UIFactory.label("Entering the frontier...",14); hud.add_child(status); cooldowns=UIFactory.label("",12,Color("#98a7c6")); hud.add_child(cooldowns); hud.add_child(UIFactory.hsep()); hud.add_child(UIFactory.label("CONTROLS",14,Color("#f0dfae"))); hud.add_child(UIFactory.label("WASD  move\nSpace  dash\nQ  rally army\nE  shockwave\n\nYour Warden auto-attacks. Your army fights around you.",12,Color("#a5b1cb"))); hud.add_child(UIFactory.spacer()); hud.add_child(UIFactory.button("Retreat to World",func():GameState.screen_requested.emit("world"),Color("#4e3337")))

func _hud(data: Dictionary) -> void:
	hp_bar.max_value=data.hp_max; hp_bar.value=data.hp
	status.text="HP %d / %d\nRun Lv.%d · Kills %d · %ds%s"%[data.hp,data.hp_max,data.level,data.kills,int(data.time),"\n★ BOSS ARRIVED" if data.boss else ""]
...
func _finished(result: Dictionary) -> void:
	GameState.expedition_completed(result); SaveManager.save_game(); result_overlay=PanelContainer.new(); result_overlay.set_anchors_preset(Control.PRESET_CENTER); result_overlay.position=Vector2(330,120); result_overlay.size=Vector2(620,450); result_overlay.add_theme_stylebox_override("panel",UIFactory.panel(Color("#111827"),15,Color("#856f48"))); add_child(result_overlay); var v=VBoxContainer.new(); result_overlay.add_child(v); var text="VICTORY · FRONTIER CLAIMED" if result.victory else "EXPEDITION BROKEN"; var tl=UIFactory.title(text,25); tl.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER; v.add_child(tl); v.add_child(UIFactory.label("Threat %d · %d kills · %d XP"%[result.threat,result.kills,result.xp],14,Color("#aebad2"))); v.add_child(UIFactory.hsep()); v.add_child(UIFactory.label("LOOT",15,Color("#f0dfae")))
	...
	v.add_child(UIFactory.spacer()); var next=UIFactory.button("Return to Settlement",func():GameState.screen_requested.emit("city"),Color("#4d563d")); next.custom_minimum_size.y=50; v.add_child(next)
