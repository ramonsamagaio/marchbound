...
func _build()->void:
	...
	var intro=UIFactory.label("Take work that bends your next expeditions toward a goal. Contracts are optional, persistent and designed to create a reason for one more run.",13,Color("#9ca8c5"))
	...
	right_margin.add_child(side_scroll)
...
func refresh()->void:
	...
	if ContractManager.available.is_empty():
		board_box.add_child(_wrap(UIFactory.label("No open writs. Complete or refresh the board.",13,Color("#8f9bb8"))))
	else:
	...
	if ContractManager.active.is_empty():
		summary.add_child(_wrap(UIFactory.label("No active contracts. Pick a writ from the board and let it shape the next march.",12,Color("#8f9bb8"))))
	else:
	...
	summary.add_child(_wrap(UIFactory.label("The contract board is a second layer over the core loop. A good contract should make a tile, boss or extra expedition suddenly feel worth doing without becoming mandatory busywork.",11,Color("#8f9bb8"))))
...
func _available_card(contract:Dictionary)->PanelContainer:
	...
	h.add_child(accept)
	v.add_child(_wrap(UIFactory.label(String(contract.description),12,Color("#9ca8c5"))))
	v.add_child(_wrap(UIFactory.label("Reward  %s · Renown +%d"%[UIFactory.cost_text(contract.reward),int(contract.renown)],11,Color("#c9b57d"))))
	return p
...
func _active_card(contract:Dictionary)->PanelContainer:
	...
	v.add_child(_wrap(UIFactory.label(String(contract.description),11,Color("#98a5c2"))))
	...
	v.add_child(actions)
	var reward=_wrap(UIFactory.label("%s · +%d Renown"%[UIFactory.cost_text(contract.reward),int(contract.renown)],10,Color("#c9b57d")))
	reward.size_flags_horizontal=Control.SIZE_EXPAND_FILL
	actions.add_child(reward)
	if ready:
