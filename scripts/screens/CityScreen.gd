...
func refresh()->void:
	...
	GameState.ensure_schema(); UIFactory.clear_children(settlement_summary); var income=GameState.resource_income_per_minute(); var row=HBoxContainer.new(); settlement_summary.add_child(row); row.add_child(UIFactory.label("Settlement Tier %d"%GameState.buildings.town_hall,17,Color("#f0dfae"))); row.add_child(UIFactory.spacer()); row.add_child(UIFactory.label("Power %d"%GameState.total_power(),15)); settlement_summary.add_child(UIFactory.label("Income/min  Gold +%d · Wood +%d · Stone +%d · Iron +%d · Food +%d · Mana +%.1f"%[income.gold,income.wood,income.stone,income.iron,income.food,income.mana],13,Color("#9fd3a7"))); settlement_summary.add_child(UIFactory.label("Frontier  %d territories claimed · Highest Threat %d · Renown %d"%[GameState.claimed_count(),GameState.world.highest_threat,GameState.player.renown],13,Color("#d5bf83")))
