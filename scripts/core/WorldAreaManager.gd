...
func ensure_schema() -> void:
	...
	ensure_area(0,0)
	_mark_discovered_radius(current_macro(),2)
...
func set_current_macro(position:Vector2i,entry_side:String="center") -> void:
	...
	Dictionary(GameState.world["areas"])[area_key(position.x,position.y)] = area
...
func can_fast_travel(target:Vector2i) -> bool:
	if target == current_macro():
		return true
	var area:Dictionary = area_state(target.x,target.y)
...
func ensure_area(x:int,y:int) -> Dictionary:
	...
	if area.is_empty():
		area = {
			...
			"home":is_home,
			"entry_side":"center",
			"player_local":[LOCAL_MAP_PX/2,LOCAL_MAP_PX/2],
			"structures":[],
			"removed_nodes":[],
			"discovered_pois":[],
			"revision":1
	...
	if is_home and Array(area["structures"]).is_empty():
		area["structures"] = _legacy_home_structures()
	areas[key] = area
...
func is_home(position:Vector2i=current_macro()) -> bool:
	return position == HOME

func is_safe(position:Vector2i=current_macro()) -> bool:
	return bool(area_state(position.x,position.y).get("safe",position==HOME))

func structures(position:Vector2i=current_macro()) -> Array:
...
func structure_at(tile_pos:Vector2i) -> Dictionary:
	for raw:Variant in structures():
		if not (raw is Dictionary): continue
		var entry:Dictionary = Dictionary(raw)
		if int(entry.get("tx",-999)) == tile_pos.x and int(entry.get("ty",-999)) == tile_pos.y:
			return entry
	return {}
...
func _mark_discovered_radius(center:Vector2i,radius:int) -> void:
	...
	for y:int in range(center.y-radius,center.y+radius+1):
		for x:int in range(center.x-radius,center.x+radius+1):
			if Vector2i(x,y).distance_to(center) <= float(radius)+0.35:
				discovered[area_key(x,y)] = true
...
func _sync_home_buildings() -> void:
	...
	for raw:Variant in structures(HOME):
		...
		if GameState.buildings.has(id):
			counts[id] = maxi(int(counts.get(id,0)),int(entry.get("level",1)))
	for raw_id:Variant in GameState.buildings.keys():
		var id:String = String(raw_id)
		if HOME_LEGACY_POSITIONS.has(id):
			GameState.buildings[id] = int(counts.get(id,0))
...
func tile_data(x:int,y:int) -> Dictionary:
	ensure_schema()
	var hashv:int = abs(hash("%s:%s:%s:%s"%[GameState.world.get("seed",947213),GameState.world.get("season",1),x,y]))
	...
	var current:Vector2i = current_macro()
	var area:Dictionary = area_state(x,y)
	return {
		...
		"visited":bool(area.get("visited",false)),"discovered":is_discovered(x,y),"current":current==Vector2i(x,y),
